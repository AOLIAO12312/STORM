...
def time_op(fn, *args, device=None, **kwargs):
    ...
    if use_cuda:
        ...
        torch.cuda.synchronize(device)
        elapsed_s = start.elapsed_time(end) / 1000.0  # ms -> s
    else:
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,
        ...
        self.scanner = CrossScanner(scan_mode=2,window_size=0,window_ratio=0.5,prune_channel=False)
        ...
        self.num_stage_layer = [2,2,8,2]
        self.num_prune_ratio = 0.1
        ...
        if self.with_dconv:
            ...
            # self.conv2d = DepthwiseConv1D(self.d_inner,5,conv_bias)
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            x_proj_bias = getattr(self, "x_proj_bias", None)
            # xs = self.scanner.cross_scan_fwd(x)
            ...
            #                            force_torch=scan_force_torch)  # generate cross scan seq
            #     else:
            #         L_truth = self.stage_size[self.stage_idx]**2 - self.layer_idx * self.num_prune[self.stage_idx]
            #         num_prune = self.num_prune[self.stage_idx]
            #         x,sorted_idx = self.merger(x.view(B, D, H * W), L_truth,num_prune)  # [B, D, L_kept]
            #         xs = self.merger.cross_scan_fwd(x)
            #         L = xs.shape[3]
            # else:
            ...
            # y : torch.Tensor = self.scanner.cross_scan_bwd(ys.view(B, K, -1, H, W))
            y: torch.Tensor = cross_merge_fn(ys.view(B, K, -1, H, W), in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)
            # TODO：执行填充，填充为0
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if not self.channel_first:
            x = x.permute(0, 3, 1, 2).contiguous()
        if self.with_dconv:
            x = self.conv2d(x) # (b, d, h, w) 2D sensitive
        x = self.act(x)
        y = self.forward_core(x) # Not 2D sensitive(proved)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y))
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.update(
            ...
            initialize=initialize, forward_type=forward_type, channel_first=channel_first,stage_idx=stage_idx,layer_idx=layer_idx,
        )
...
class DownsampleModule(nn.Module):
    def __init__(self, dim, out_dim,kernel_size):
        ...
        self.out_dim = out_dim
        if kernel_size == 2:
            self.conv = nn.Conv2d(self.dim, self.out_dim, kernel_size=2, stride=2)
        elif kernel_size == 3:
            self.conv = nn.Conv2d(self.dim, self.out_dim, kernel_size=3, stride=2,padding=1)
        elif kernel_size == 5:
            self.conv = nn.Conv2d(self.dim, self.out_dim, kernel_size=5, stride=2,padding=2)
        elif kernel_size == 1:
            self.conv = nn.Conv2d(self.dim, self.out_dim, kernel_size=1,stride=2)
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.num_layers = len(depths) # [2,2,8,2] - tiny
        ...
        self.pos_embed = self._pos_embed(dims[0], patch_size, imgsize) if posembed else None # None 不进行位置编码
        ...
        self.layers = nn.ModuleList() # Core Layer
        for i_layer in range(self.num_layers):
            downsample = _make_downsample( # DownSample
                self.dims[i_layer],
                self.dims[i_layer + 1],
                norm_layer=norm_layer,
                channel_first=self.channel_first, # 用于兼容
            ) if (i_layer < self.num_layers - 1) else nn.Identity() # 最后一层为 nn.Identity()

            self.layers.append(self._make_layer( # VSS Block
    ...
    #     return nn.Sequential(
    #         (nn.Identity() if channel_first else Permute(0, 3, 1, 2)),
    #         DownsampleModule1D(dim, out_dim,kernel_size=5),  # 使用 DownsampleModule 来替代 downsample 函数
    #         (nn.Identity() if channel_first else Permute(0, 2, 3, 1)),
    #         norm_layer(out_dim),
    #     )
    ...
    def flops(self, shape=(3, 224, 224), verbose=True):
        ...
        def norm_flop_jit(inputs, outputs):
            """FLOPs for aten::linalg_vector_norm (i.e., torch.norm(..., dim=-1))"""
            input_shape = get_shape(inputs[0])
            C = input_shape[-1]  # feature dim
            num_vectors = np.prod(input_shape[:-1])  # N * T_pair
            # sum(x^2): C muls + (C-1) adds ≈ 2C
            # sqrt: 1 FLOP
            return num_vectors * (2 * C + 1)

        def sqrt_flop_jit(inputs, outputs):
            """FLOPs for aten::sqrt"""
            input_shape = get_shape(inputs[0])
            return np.prod(input_shape)  # 1 FLOP per element

        def sum_flop_jit(inputs, outputs):
            """FLOPs for aten::sum (reduction)"""
            input_shape = get_shape(inputs[0])
            output_shape = get_shape(outputs[0])
            # Reduction: (input_numel - output_numel) adds
        ...
        supported_ops={
            ...
            "aten::linalg_vector_norm": norm_flop_jit,  # for cosine
            "aten::sqrt": sqrt_flop_jit,                # for L2
            "aten::sum": sum_flop_jit,                  # for all distances
            "aten::sub": sub_flop_jit,                  # for L1/L2
            "aten::mul": mul_flop_jit,                  # for cosine (a_n * b_n)
            "aten::abs": lambda inputs, outputs: 0,     # abs() not counted as FLOP
        }
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        ...
        # --------- NEW: align outputs to FPN-expected pyramid sizes (nearest) ---------
        # Use the first output (e.g., C2) as reference.
        # Expect strict /2 pyramid: (H2,W2) -> (H2//2,W2//2) -> ...
        if len(outs) >= 2:
            H2, W2 = outs[0].shape[-2], outs[0].shape[-1]

            target_hw = [(H2, W2)]
            for _ in range(1, len(outs)):
                ph, pw = target_hw[-1]
                target_hw.append((ph // 2, pw // 2))

            aligned = [outs[0]]
            for k in range(1, len(outs)):
                th, tw = target_hw[k]
                cur = outs[k]
                ch, cw = cur.shape[-2], cur.shape[-1]
                # only resize if mismatch
                if (ch, cw) != (th, tw):
                    cur = F.interpolate(cur, size=(th, tw), mode='nearest')
                aligned.append(cur.contiguous())
            outs = aligned
        # ---------------------------------------------------------------------------
