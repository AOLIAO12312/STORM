...
class ToMe2Dv2(nn.Module):
    def __init__(
        ...
        eps: float = 1e-6,
        # >>> 新增：重要性引导相关参数 <<<
        use_importance: bool = False,      # 是否开启重要性引导
        imp_lambda: float = 0.7,           # 惩罚强度 λ
        imp_mode: str = 'l2',              # 'l2' | 'l1' | 'mean_abs'
        imp_detach: bool = True            # 是否从计算图中 detach 重要性（更稳）
    ):
        super().__init__()
        ...
        self.imp_lambda = imp_lambda
        self.imp_mode = imp_mode
        self.imp_detach = imp_detach
    ...
    def _pair_scores(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        ...
        if self.distance == 'cosine':
            ...
            scores = (a_n * b_n).sum(dim=-1)
        elif self.distance == 'l1':
            scores = - (a - b).abs().sum(dim=-1)
        elif self.distance == 'l2':
            scores = - ((a - b) ** 2).sum(dim=-1).sqrt()
        else:
        ...
        if self.use_importance:
            if self.imp_mode == 'l2':
                imp_a = a.pow(2).sum(dim=-1)          # [N, T_pair]
                imp_b = b.pow(2).sum(dim=-1)
            elif self.imp_mode == 'l1':
                imp_a = a.abs().sum(dim=-1)
                imp_b = b.abs().sum(dim=-1)
            elif self.imp_mode == 'mean_abs':
                imp_a = a.abs().mean(dim=-1)
                imp_b = b.abs().mean(dim=-1)
            else:
                raise ValueError(f"Unsupported imp_mode {self.imp_mode}")

            # pair 重要性：只要这对里有一个很重要，就偏向不合并
            imp_pair = torch.max(imp_a, imp_b)  # [N, T_pair]

            if self.imp_detach:
                imp_pair = imp_pair.detach()

            # 归一化，避免对不同层 / 不同 backbone 影响尺度差异过大
            imp_pair = imp_pair / (imp_pair.mean(dim=-1, keepdim=True) + self.eps)

            # 惩罚高重要 pair（cosine / -L1 / -L2 都适用）
            scores = scores - self.imp_lambda * imp_pair
        # ===========================
...
if __name__ == '__main__':
    ...
    merge2d = ToMe2Dv2(if_order=True, distance='cosine', merge_mode='sum')
