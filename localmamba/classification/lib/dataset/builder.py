...
def build_dataloader(args):
    ...
    if args.dataset == 'imagenet':
        # 让你传的路径生效；不传就用默认
        args.data_path = args.data_path or '/home/Vim-main/data/ImageNet-1K'
    ...
    if args.dataset == 'imagenet':
        train_transforms_l, train_transforms_r = transform.build_train_transforms(
            args.aa, args.color_jitter, args.reprob, args.remode,
            args.interpolation, args.image_mean, args.image_std)

        train_dir = os.path.join(args.data_path, 'train')
        train_dataset = datasets.ImageFolder(train_dir, transform=train_transforms_l)
    ...
    if args.dataset == 'imagenet':
        ...
        val_dataset = datasets.ImageFolder(val_dir, transform=val_transforms_l)

    elif args.dataset == 'cifar10':
        val_transforms_l, val_transforms_r = transform.build_val_transforms_cifar10(
            args.image_mean, args.image_std)
        val_dataset = datasets.CIFAR10(
            root=args.data_path, train=False, download=True, transform=val_transforms_l)
