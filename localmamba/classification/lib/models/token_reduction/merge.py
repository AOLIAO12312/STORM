...
class RMeeTo_Merge(nn.Module):
    ...
    def forward(self, metric: torch.Tensor, token_position: int, num_prune:int) -> Callable:
        """w
        ...
        """
        self.change_num_prune(num_prune)
        ...
        def do_noting(x: torch.Tensor, mode=None) -> Tuple[torch.Tensor, int,None]:
            return x, token_position,None
        ...
        def ordered_merge(x: torch.Tensor, mode="mean") -> Tuple[torch.Tensor, int,torch.Tensor]:
            n, t, c = x.shape
            cls_token = x[:, token_position, :]
            x = torch.cat([x[..., :token_position, :], x[..., token_position + 1:, :]], dim=1)
            src, dst = x[..., ::2, :], x[..., 1::2, :]
